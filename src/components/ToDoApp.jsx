...
export default class ToDoApp extends Component {
  ...
  isValidToDo = (todo) => {
    return (
      todo.item.length &&
      !this.state.todoList.some((t) => t.item.toLowerCase() === todo.item.toLowerCase() && t.id != todo.id)
    )
  }
  ...
  render() {
    ...
    return (
      <div className="todo-container">
        ...
        <button className="add-btn" onClick={this.addToDo} disabled={this.state.todo.length <= 0}>
          Add
        </button>
        {this.state.error && <div className="error">{this.state.error}</div>}
        <div className="header">TODO</div>
        {inProgressToDoItems.length ? (
          inProgressToDoItems.map((todo, i) => (
            <ToDoItem
              todo={todo}
              key={i}
